...
- Exchange name/type is centrally configured via `Messaging:Topology`:
    - `Messaging:Topology:EventsExchangeName` (default: `orders.events-exchange`)
    - `Messaging:Topology:EventsExchangeType` (default: `topic`)

### Exchange Suffix Rule (`-exchange`)

- The `-exchange` suffix is mandatory for the shared domain-events exchange.
- Queue endpoint names remain queue-oriented (`*-queue`) by design.

Why command/read-model endpoints are not renamed to `*-exchange`:

- Saga command routing is queue-address based (`queue:<name>`) and must remain deterministic.
- Forcing endpoint/exchange renames would couple endpoint identity to a broker naming migration and can break existing routing and tests.
- MassTransit creates endpoint exchanges as part of receive endpoint topology; these are transport artifacts, not domain event contract names.
